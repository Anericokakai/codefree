...
export const add_blog = async (req, res) => {
  const { title, author, illustration, topic, id, image ,updateblog} = req.body;

  console.log(req.body)
if(req.body.updateblog){
  console.log('true')

  const updateBlog=await blogs_collection.findByIdAndUpdate(id,{
    tittle:title,
  ...
  if(!updateBlog){
    return res.json({error:'failed to update the blog'})
    console.log('failed to update')
  } 
return res.json({status:'ok',success:'blog updated succsefully'})
...
}else
{
  ...
  if (!topics_collection_data) {
    ...
    if (newBlog) {
      res.json({ status: "ok", success: "blog added succsefully" });
    }
...
export const addcourse_helper = async (req, res) => {
  ...
  if (todelete) {
    ...
    if (!couse_id_finder)
      return res.json({ error: "the course is not available" });
    ...
    if (delete_topics)
      return res.json({
        error: "failed  to delete the Topics under this course first",
      });
    ...
    if (!delete_document) return res.json({ error: "failed to delete blog" });
