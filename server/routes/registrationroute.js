...
const refresh_key=process.env.REFRESH_SECRETE_KEY
  ...
  res.json({ status: "ok", token: token, name: db_name,refresh_token:refresh_token });
};
...
const valid_refresh_token=await jwt.verify(refresh_token,refresh_key,(err,payload)=>{
  if(err){
    console.log('error the json token is invalid')
  }
  if(payload){
    console.log('token is made valid')
    const {email}=payload
     const token = jwt.sign({ email: email }, secrete_key,{
     expiresIn:'10s'
   })
