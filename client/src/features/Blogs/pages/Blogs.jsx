...
function Blogs() {
  ...
  return (
    <div
      className={`bg-primary min-h-[100vh] grid font-poppins  justify-center px-2`}
    >
          ...
          <section className="   bg-black-gradient text-dimWhite sm:min-h-[11rem] min-h-[9rem] rounded-xl  grid place-items-center max-w-[60rem] md:min-w-[60rem] sm:min-w-[45rem] relative w-full ">
            ...
            <div className="absolute sm:-bottom-6 -bottom-7 w-[90%] bg-[#242c38]  flex rounded-xl ">
              <div className="flex w-full rounded-xl border border-slate-700 relative">
                ...
                <div
                  className={` ${focus ? "block" : "hidden"} absolute border w-full top-[4rem]  xs:top-[5rem] max-h-[20rem] overflow-y-scroll  min-h-[15rem] z-20 bg-[#242c38] rounded-xl border-slate-600`}
                >
                  {searchResults.map((item, index) => {
                    return (
                      <div
                        className="flex p-2 hover:bg-[#434a57] cursor-pointer "
                        key={index}
                      >
                        <p
                          className={`${styles.paragraph} text-[1.125rem] sm:text-[1.2rem] `}
                        >
                          {item}
                        </p>
                      </div>
